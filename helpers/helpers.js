...
HTMLElement.prototype.scrollTo = function( containedElement ) {
  "Scrolls viewport to center around argument DOM element"
  containedElement.scrollIntoViewIfNeeded()
}
...
// move tests from the repl to a tests array, over time...
Element.test = function(){
  var n = Element('pre', {})
  assert(n.innerText === '', 'No inner text')
  var m = Element('pre', {}, ['text'])
  assert(m.innerText === 'text', 'Inner text')
  return 'ok'
}


var assert = function (bool, desc) {
  if( !bool) throw desc;
}
